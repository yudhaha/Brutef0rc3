...
class BFF:
    ...
    def run(self):
        ...
        if lib.isActive(self) == False:
            ...
            br(1)
            inputs('!k!enter untuk login kembali..', blank_left=4)
            return self.run()
        ...
        try:
            br(1)
            pils = int(inputs('!p!yudhaha/>!b! ', blank_left=4))
            pils = '%02d'%(pils,)
    ...
    def back(self):
        inputs('!k!enter untuk kembali..', blank_left=4)
        return self.run()

    def clearDumpCache(self, count=0):
        list = lib.cachecrackList()
        if len(list) == 0:
            br(1)
            ...
            return self.back()
        br(1)
        prints('!m![ !b!ini hasil cracknya!r! !m!]', blank_left=4)
        br(1)
        for cache in list:
            count+=1
            num = '%02d'%(count,)
