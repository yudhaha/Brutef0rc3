...
class Dump:
    ...
    def save(self, saveTo):
        ...
        save = open('crack/%s.json'%(saveTo), 'w')
        ...
        save.close()
        br(2)
        prints('!h!Hasil tersimpan : !p!crack/%s.json'%(saveTo), blank_left=4)
        prints('!h!ID didapatkan     : !p!%s' %(len(self.__id__)), blank_left=4)
        br(1)
    ...
    def react(self, path=None):
        prints('!p!Contoh link postingan !m!(!b!https://www.facebook.com/photo.php?fbid=193850482185404&set=a.110623187174801&type=3&app=fbl!m!)', blank_left=4)
        br(1)
        while True:
            try:
