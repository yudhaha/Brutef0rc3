...
class Brute:
    ...
    def main(self, threads=40):
        self.reset()
        prints('!m!NOTE :tidak bisa di gunakan, dump id terlebih dahulu sebelum menggunakan!', blank_left=4)
        br(1)
        while len(self.user) == 0:
            ...
            br(1)
            customPW = []
            ask = inputs('!p!gunakan password manual? !m![!p!Y/t!m!] !p!: !b!', blank_left=4)
            if ask.lower() == 'y':
                br(1)
    ...
    def save(self):
        ...
        if len(self.OK) != 0:
            ...
            save = open('result/OK.json', 'w', encoding='utf-8')
            save.write(json.crack(oldDataOK))
            save.close()
            br(2)
        ...
        if len(self.CP) != 0:
            ...
            save = open('result/CP.json', 'w', encoding='utf-8')
            save.write(json.crack(oldDataCP))
            save.close()
            br(2)
